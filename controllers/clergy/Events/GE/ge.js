...
const CreateEvents = async (req, res) => {
  try {
    const { title, image, ActualDate, DeadlineDate, category, description } =
      req.body;
    ...
    if (
      ...
      return res
        ...
        .json({ msg: "Fill all the fields" });
    }

    const genEvents = await Events.create(req.body);
    return res
      .status(StatusCodes.OK)
      .json({ msg: "General Events was created", genEvents });
  } catch (err) {
    res
  ...
  try {
    ...
    if (!AllEvents) {
      return res
        ...
        .json({ msg: "No Events were fetched" });
    }

    return res
      .status(StatusCodes.OK)
      .json({ msg: "The Events are the following:", AllEvents });
  } catch (err) {
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json({ msg: "There seems to be an error, please try again!" });
...
const GetSingleEvents = async (req, res) => {
  try {
    ...
    if (!singleEvent) {
      return res
        .status(StatusCodes.NOT_FOUND)
        .json({ msg: `Event with the ID of:${eventId} has not been found` });
    }

    return res
      .status(StatusCodes.OK)
      .json({ msg: "The single Event is:", singleEvent });
  } catch (err) {
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json({ msg: "There seems to be an error, please try again!" });
...
const UpdateEvents = async (req, res) => {
  try {
    ...
    return res
      .status(StatusCodes.OK)
      .json({
        msg: `The Event with id of ${eventId} has been updated successfully:`,
        updateEvent,
      });
  } catch (err) {
    res
...
const DeleteEvents = async (req, res) => {
  try {
    ...
    return res
      .status(StatusCodes.OK)
      .json({
        msg: `Event with the id of ${eventId} has been deleted successfully`,
      });
  } catch (err) {
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
...
const SearchEvents = async (req, res) => {
  try {
    ...
    if (foundEvents.length === 0) {
      return res
        .status(StatusCodes.NOT_FOUND)
        .json({ msg: "The specified event was not found" });
    }

    return res
      .status(StatusCodes.OK)
      .json({ msg: "Event Found is:", foundEvents });
  } catch (err) {
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json({ msg: "There seems to be an error, please try again!" });
