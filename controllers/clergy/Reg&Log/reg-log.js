...
const Register = async (req, res) => {
  try {
    ...
    return res
      ...
      .json({ msg: "Clergy Registered Successfully", sentCreatedClergy });
  } catch (err) {
    res
...
const Login = async (req, res) => {
  try {
    ...
    if (!email || !password) {
      return res
        .status(StatusCodes.BAD_REQUEST)
        .json({ msg: "Provide all the Details required" });
    }
    ...
    if (!clergyEmail) {
      return res
        ...
        .json({ msg: "The Email you provided cannot be found" });
    }
    ...
    const token = jwt.sign(
      { clergyId: clergyEmail._id },
      process.env.clergy_key,
      { expiresIn: "1d" }
    );

    return res
      .status(StatusCodes.OK)
      .json({ msg: `Login Successful`, clergyLogin, clergyToken: token });
  } catch (err) {
    console.log(err);
...
const UpdateProfile = async (req, res) => {
  try {
    ...
    const updateClergy = await AuthModel.findOneAndUpdate(
      { _id: clergyId },
      req.body,
      {
        new: true,
    ...
    if (!updateClergy) {
      return res
        .status(StatusCodes.NOT_FOUND)
        .json({ msg: `Staff with id:${clergyId} has not been found` });
    }

    res
      .status(StatusCodes.OK)
      .json({
        msg: `Your Details have been updated succesfully`,
        updateClergy,
      });
  } catch (err) {
    console.log(err);
